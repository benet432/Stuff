  ...
  
{
  ...
  pi=-0.5*matrix(beta.star-beta.0, 1, p)%*%
    ...
    matrix(y, 1, l)%*%X%*%matrix(beta.star,p,1)-
    matrix(m, 1, l)%*%log(1+exp(X%*%matrix(beta.star,p,1)))
  return(pi)
}
                           ...
                           retune=200, verbose=TRUE)
{
  ...
  while (verbose)
  {
    ...
    { 
      verbose = FALSE
      cat("Accept Rate is: ",accept_rate," We take v^2 as: ",
          Sigma_new[1, 1], "\n")
    ...
    else
    {
      verbose = TRUE
      Sigma_new=Sigma_new*exp(0.5)
      cat("Accept Rate is: ",accept_rate," we adjust v^2 to: ", 
          Sigma_new[1, 1], "\n")
    }  
    }
